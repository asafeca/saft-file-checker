...
var SaftValidation = /** @class */ (function () {
  ...
  SaftValidation.checkHeaderAttributeList = function (_a) {
    var attributeList = _a.attributeList,
    ...
    for (var _i = 0, _b = Object.keys(attributeList); _i < _b.length; _i++) {
      ...
      if (!result.success) {
        return new iresult_1.DataResult({
          message: 'Element '.concat(attribute.getName(), ' Inv\u00E1lido'),
          success: false,
        });
      }
    }
  ...
  SaftValidation.checkHeaderElement = function (_a) {
        ...
        if (element.getName() === currentNode.nodeName) {
          if (element.getName() === 'CompanyAddress') {
            ...
            if (!result.success) {
              return new iresult_1.DataResult({
                success: false,
                message: ''.concat(element.getName(), ' n\u00E3o encontrado'),
              });
            }
          }
          ...
          if (!isContentValid) {
            return new iresult_1.DataResult({
              message: 'Elemento '.concat(element.getName(), ' com valor inv\u00E1lido'),
              success: false,
            });
          }
    ...
    return new iresult_1.DataResult({
      success: false,
      message: 'Ficheiro Inv\u00E1lido. '.concat(element.getName(), ' n\u00E3o encontrado'),
    });
  };
  ...
  SaftValidation.checkCompanyAddressAttrElement = function (_a) {
    ...
    return new iresult_1.DataResult({
      success: false,
      message: 'Ficheiro Inv\u00E1lido. '.concat(attribute.getName(), ' n\u00E3o encontrado'),
    });
  };
  SaftValidation.prototype.childNodeChildrenMatch = function (_a) {
    ...
    for (var _i = 0, _b = Object.keys(matchList); _i < _b.length; _i++) {
      ...
      if (!this.isElementValid({ element: taxEntry, childNode: childNode })) {
        return new iresult_1.DataResult({
          success: false,
          message: 'Ficheiro inv\u00E1lido. ['.concat(taxEntry.getName(), '] n\u00E3o encontrado'),
        });
      }
    }
  ...
  SaftValidation.isAttributeInTheNode = function (_a) {
      ...
      for (var _i = 0, _b = Object.keys(nodeList); _i < _b.length; _i++) {
        ...
        if (attribute.getName() == currentNode.nodeName) {
          if (!attribute.getIsParent()) {
            ...
            if (!isContentValid) {
              return new iresult_1.DataResult({
                message: 'Elemento '.concat(attribute.getName(), ' com valor inv\u00E1lido'),
                success: false,
              });
            }
          }
    ...
    return new iresult_1.DataResult({
      success: false,
      message: 'Elemento inv\u00E1lidoo ['.concat(attribute.getName(), '] n\u00E3o encontrado!'),
    });
  };
