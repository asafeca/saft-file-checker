    ...
    if (!(sourceNodeList.length - 1 <= 0)) {
      ...
      for (var _i = 0, _b = Object.keys(sourceNodeList); _i < _b.length; _i++) {
        ...
        if (currentNode.nodeName === 'SalesInvoices') {
          ...
          var salesResult = sales_invoices_1.SalesInvoicesValidation.isSalesInvoicesValid({
            salesInvoicesNodeList: salesInvoicesNodeList,
          });
          if (!salesResult.success) {
            return salesResult;
          }
        } else if (currentNode.nodeName === 'WorkingDocuments') {
          var result = working_documents_1.WorkingDocuments.isWorkingDocumentsValid({
            workingNodes: currentNode.childNodes,
          });
          if (!result.success) {
            return result;
          }
