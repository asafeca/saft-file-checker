...
import { CurrencyValidation } from "../source_documents/invoices/document_totals/currency";
...
export class SaftValidation {
     ...
     static checkHeaderAttributeList({ attributeList, nodeList }:
          ...
          for (let keys of Object.keys(attributeList)) {

               const attribute = attributeList[keys as any]

               const result = this.checkHeaderElement({ element: attribute, nodeList: nodeList })
     ...
     private static checkHeaderElement({ element, nodeList }:
               ...
               if (currentNode.nodeName !== undefined && currentNode.nodeName !== "#text")
                    ...
                    if (element.getName() === currentNode.nodeName) {
                         ...
                         const isContentValid = ContentTypeValidation.isContentValid({
                              typeModel: element, content: currentNode.textContent !== null ? currentNode.textContent : ""
                         })
                         if (!isContentValid) {
     ...
     private static checkCompanyAddressAttr(nodeList: NodeListOf<ChildNode>): IDataResult {
          if (!(nodeList.length - 1 <= 0)) {
               ...
               let addressAttributes = SaftAttributeList.CompanyAddressAttributes

               for (let attrKey of Object.keys(addressAttributes)) {
     ...
     private static checkCompanyAddressAttrElement({ attribute, nodeList }:
     ...
     childNodeChildrenMatch({ childNode, matchList }: { childNode: ChildNode, matchList: Array<SaftAttributeModel> }): IDataResult {
     ...
     isElementValid({ element, childNode }:
          { element: SaftAttributeModel, childNode: ChildNode }): IDataResult {
     ...
     static verifyAttributesInTheNodes({ attributes, nodeList }:
          { attributes: Array<SaftAttributeModel>, nodeList: NodeListOf<ChildNode> }): IDataResult {
     ...
     private static isAttributeInTheNode({ attribute, nodeList }:
          { attribute: SaftAttributeModel, nodeList: NodeListOf<ChildNode> }): IDataResult {
