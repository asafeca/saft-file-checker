...
export class CustomerValidation {
  ...
  private static checkCustomerChildren({
    ...
    customerAttributes: SaftAttributeModel[];
  }): IDataResult {
    for (const attrKey of Object.keys(customerAttributes)) {
  ...
  private static isAttributeInTheNode({
    ...
    for (const nodeKey of Object.keys(customerNodeList)) {
      ...
      if (attribute.getName() === currentNode.nodeName) {
        if (attribute.getName() === 'BillingAddress' || attribute.getName() === 'ShipToAddress') {
          ...
          const addressResult = this.checkCustomerChildren({
