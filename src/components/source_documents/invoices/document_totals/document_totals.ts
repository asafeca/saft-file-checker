...
export class DocumentTotalsValidation {
  static isDocumentTotalsValid({ totalsNodes }: { totalsNodes: NodeListOf<ChildNode> }): IDataResult {
    ...
    if (!currencyExists) {
      ...
      const totalsResult = SaftValidation.verifyAttributesInTheNodes({ attributes, nodeList: totalsNodes });
    ...
    // CURRENCY EXISTS
      ...
      if (!currencyResult.success) {
        ...
        for (const totalKey of Object.keys(totalsNodes)) {
          ...
          if (currentNode.nodeName === 'Currency') {
            ...
            if (!(currencyNodes.length - 1 <= 0)) {
              ...
              const currencyResult1 = SaftValidation.verifyAttributesInTheNodes({
