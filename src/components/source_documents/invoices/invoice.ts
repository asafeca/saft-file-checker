...
export class InvoiceValidation{
   static isInvoiceValid({invoiceNodeList}:{invoiceNodeList:NodeListOf<ChildNode>}):IDataResult{
    ...
    const invoiceResult = SaftValidation.verifyAttributesInTheNodes(
        {attributes: invoiceAttributes, nodeList: invoiceNodeList})

        if(!(invoiceResult.success)){
            return invoiceResult
        }

        for(let nodeKey of Object.keys(invoiceNodeList)){
            ...
            if(currentNode.nodeName !== "#text" && currentNode.nodeName !== undefined){
                if(currentNode.nodeName === "DocumentStatus" || currentNode.nodeName === "SpecialRegimes"){
                    ...
                    const shipResult = SaftValidation.verifyAttributesInTheNodes({nodeList: shipNodes, attributes: shipAttributes})
                    if(!shipResult.success){
                        return shipResult
                    }

                    for(let shipKey of Object.keys(shipNodes)){
                ...
                const invoiceResult = LineValidation.isLineValid({lineNodes: lineNodes,invoiceType: invoiceType })
   ...
   else{return new DataResult({message: `Elemento [Invoice] inválido`, success:false})}
    }


    private static getCorrectAttributeList(attribute:string):Array<SaftAttributeModel>{
        switch(attribute){
            case "DocumentStatus":
            ...
            default: return new Array<SaftAttributeModel>()
    ...
    private static getInvoiceType({invoiceNodeList}:{invoiceNodeList:NodeListOf<ChildNode>}):SaftAttributeModel{
        ...
        for(let nodeKey of Object.keys(invoiceNodeList)){
            ...
            if(currentInvoice.nodeName === "InvoiceType"){
                ...
                return new SaftAttributeModel({name: name, isParent:false, type:"string"})
            }
        }
