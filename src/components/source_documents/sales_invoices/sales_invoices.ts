...
export class SalesInvoicesValidation{
    ...
    static isSalesInvoicesValid({salesInvoicesNodeList}:
        {salesInvoicesNodeList:NodeListOf<ChildNode>}):IDataResult{
        ...
        const salesResult = SaftValidation.verifyAttributesInTheNodes(
            {attributes: SalesInvoicesAttributes, nodeList: salesInvoicesNodeList})

            if(!salesResult.success){
                return salesResult
            }

            for(let nodeKey of Object.keys(salesInvoicesNodeList)){
